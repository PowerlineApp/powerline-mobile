angular.module('app.services').factory('userPetitions',function ($q, UserPetitionsResource, iStorage, formatOptions,
  ...
  return {
    ...
    load: function (id) {
      var deferred = $q.defer();
      UserPetitionsResource.get({id: id}, function (data) {
        setOwner(data);
        formatInfo(data);
        deferred.resolve(data);
      }, function () {
...
}).factory('UserPetitionsResource', function ($resource, serverConfig) {
  return $resource(serverConfig.url + '/api/v2/user-petitions/:id', {id: '@id'}, {
    searchByHashTag: {
      method: 'GET',
      isArray: true,
